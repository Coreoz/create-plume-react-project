...
export default class UsersList {
  ...
  render = ({ usersWithRoles, usersPath }: Props) => {
    ...
    return (
      <>
        <this.theme.panelTitle>
          {messages.t('user.title_list')}
        </this.theme.panelTitle>
        <this.theme.panelContent>
          <this.theme.panelContentElement columns={5}>
            <this.theme.panelContentElementColumn width={4}>
              {usersWithRoles && (
                <table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {usersWithRoles.users.map((user: AdminUserDetails) => (
                      <tr key={user.id}>
                        <td><Link
                          to={`${usersPath}/${user.id}`}>{user.userName}</Link>
                        </td>
                        <td><Link
                          to={`${usersPath}/${user.id}`}>{user.email}</Link></td>
                        <td><Link
                          to={`${usersPath}/${user.id}`}>{user.firstName}</Link>
                        </td>
                        <td><Link
                          to={`${usersPath}/${user.id}`}>{user.lastName}</Link>
                        </td>
                        <td><Link
                          to={`${usersPath}/${user.id}`}>{usersWithRoles.roles.get(user.idRole)}</Link>
                        </td>
                        <td><Link
                          to={`${usersPath}/${user.id}`}>{dayjs(user.creationDate).format('L LT')}</Link>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          </this.theme.panelContentElement>
        </this.theme.panelContent>
      </>
    );
  };
