...
import { ROUTE_USERS_CREATE, ROUTE_USERS_UPDATE, userRoutes } from '@lib/plume-admin-users/router/UserRouter';
      ...
      <SearchLayout>
        <SearchHead>
          ...
          <SearchActions>
            <ActionLink
              icon="add"
              linkTo={userRoutes[ROUTE_USERS_CREATE]}
            ...
            >
              {
                displayedItems.map((user: AdminUserDetails) => (
                  <ListItem
                    key={user.id}
                    onClick={() => userRoutes[ROUTE_USERS_UPDATE]({ userId: user.id }).push()}
                  >
                    <UserTile user={user} />
                  </ListItem>
                ))
